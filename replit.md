...
- **3-Level Cascading Filters (Phase 2 Complete)**: Full hierarchical filter system supporting Make→Model→Trim patterns for Vehicles, Brand→Model→Variant for Mobile Phones, etc. Database-backed `attribute_options` table with parent-child relationships. Admin UI for managing hierarchical options. `dependent_select` field type in category schemas. Cascading select components for product create/edit forms with proper state management, refetch guards, and UNSET_VALUE handling. Search page filter sidebar supports dependent dropdowns with automatic parent-child option loading. **Storage Contract**: Cascading selects store display values (option.value) NOT IDs, matching existing product attribute storage pattern. IDs are only used for fetching child options.
...
- **Site Content Management System**: Database-backed admin control over branding, promotional banners, hero slideshow, footer links, homepage sections, and static pages with full CRUD. Hero slides and promotional banners support both images and videos (MP4/WebM, 50MB limit) with media type selector, autoplay/muted/loop playback, and poster image fallback.
