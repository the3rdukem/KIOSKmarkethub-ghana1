...
- **Phase 7D Courier-Assisted Delivery**: Vendors manage order-level delivery workflow via inline action buttons in order details dialog. Flow: `preparing` → `ready_for_pickup` → `out_for_delivery` → `delivered` → (48h auto) → `completed`. Courier selection modal supports Bolt/Uber/Yango/Qargo/Other with optional booking reference (no rider PII stored). After dispatch confirmation, courier app/website opens in new tab with pre-filled delivery address where supported (Uber supports deep links with address pre-fill; Bolt/Yango/Qargo open homepage for manual entry). Admin endpoint `/api/admin/orders/auto-complete` processes orders in `delivered` status >48h for automatic completion. All transitions use `transitionOrderStatus()` with idempotent handling for race conditions, actor permissions, audit logging, and buyer notifications.
