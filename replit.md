...
- **Site Content Management System**: Fully database-backed site content management allowing admin control over branding (logo, site name, tagline), promotional banners (toggle, text, link), hero images (Base64, 1MB limit), footer links (custom ordering/visibility), homepage section titles (categories, featured, stats, CTA sections), and static pages (About, Privacy, Terms, Contact, Mobile Money, Security) accessible at `/pages/[slug]`. All content changes take effect without code deployment.
